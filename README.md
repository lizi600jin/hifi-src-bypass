...
> ### ⚠️ 不可与「op13 专用模块」共存
>
> 本模块与本人的 [op13-hifi-src-bypass](https://github.com/lizi600jin/op13-hifi-src-bypass)
> **不能同时安装**。二者的目标完全重叠 —— 都会在同一批音频策略文件、同一批 USB HAL 库上落补丁。
> 同时存在时，后挂载的那一份会把先前的顶掉；而「一键还原」各自只清理自己那一次挂载，
> 最终留下一堆**互相覆盖、归属不明**的绑定挂载，表现为音频异常、无声，或设置反复失效。
>
> **正确做法**：先在 root 管理器里卸载（或至少彻底停用）op13 专用模块，**重启**，
> 再安装本模块。安装脚本检测到旧模块仍存在时会给出警告 —— 请不要忽略它。
